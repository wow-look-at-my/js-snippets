...
scripts/run-ste-lint.mjs ← drives `wow-look-at-my/actions@ste-lint#latest` (the org's ASD-STE100 mechanical-subset prose gate) with the same inputs deploy.yml's `ste-lint` job passes, so a local run and CI apply identical rules. Six rules FAIL (hard-wrapped paragraphs, semicolons, sentences over 25 words, should/shall/could/might/would, comma splices, contractions); everything else is a warning. `scripts/ste-lint-summary.mjs <captured-output>` decodes a run into counts per rule and per file. `scripts/unwrap-prose.mjs`, `scripts/split-semicolons.mjs` and `scripts/fence-type-blocks.mjs` do the mechanical half of a conversion — all three are heuristics over hand-written files, so READ THE DIFF: they have mangled signature/description pairs and inline code before now. STILL TO CONVERT (counts are current failures, not exemptions): src/ui/llms.txt (445), src/webgpu/llms.txt (51), src/apng/llms.txt (26), src/editor/llms.txt (15) — add each to the job's `files` input as it lands
scripts/lint-yaml-comment-runs.mjs ← local mirror of the org's `yaml-comment-block` gate (max ONE comment-only line in a row in a workflow file), so a comment edit is checked before the push instead of by a CI round trip. Run: `node scripts/lint-yaml-comment-runs.mjs`
