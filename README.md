...
Branches that touch the timeline chart (`src/ui/`) or `showcase/` also publish a live single-file demo of `<timeline-view>` (fake local data, every visual looping) to buildhost: `https://sites.pazer.build/timeline-showcase/branch/<branch>/` (`/` in branch names flattened to `-`). Build it locally with `pnpm build:showcase` → `showcase/dist/index.html`.
